...
define("ACTIVE_API", "https://active.shopimint.com/api/v1/active");
define("DEACTIVE_API", "https://active.shopimint.com/api/v1/deactive");
...
function verifyPurchaseCode($code)
{
    ...
    if ($success) {
        update_option("shopimint_purchase_code", true);
        update_option("shopimint_purchase_code_key", $code);
    } else {
...
{
    $serverKey = get_option("shopimint_firebase_server_key");
...
function sendNotificationToUser($userId, $orderId, $previous_status, $next_status)
{
    ...
    $deviceToken = get_user_meta($userId, 'shopimint_device_token', true);
    $title = get_option("shopimint_status_order_title");
    if (!isset($title) || $title == false) {
        $title = "Order Status Changed";
    }
    $message = get_option("shopimint_status_order_message");
...
function sendNewOrderNotificationToDelivery($order_id, $status)
{
    ...
    if (is_plugin_active('wc-frontend-manager-delivery/wc-frontend-manager-delivery.php')) {
        if ($status == 'cancelled' || $status == 'refunded') {
            ...
            foreach ($result as $item) {
                ...
                $deviceToken = get_user_meta($item->delivery_boy, 'shopimint_delivery_device_token', true);
                if (isset($deviceToken) && $deviceToken != false) {
                    pushNotification($title, $message, $deviceToken);
                }
            }
    ...
    if (is_plugin_active('delivery-drivers-for-woocommerce/delivery-drivers-for-woocommerce.php')) {
        ...
        if ($driver_id) {
            ...
            $deviceToken = get_user_meta($driver_id, 'shopimint_delivery_device_token', true);
            if (isset($deviceToken) && $deviceToken != false) {
                pushNotification($title, $message, $deviceToken);
...
function sendNewOrderNotificationToVendor($order_seller_id, $order_id)
{
    ...
    $title = get_option("shopimint_new_order_title");
    if (!isset($title) || $title == false) {
        $title = "New Order";
    }
    $message = get_option("shopimint_new_order_message");
    if (!isset($message) || $message == false) {
        $message = "Hi {{name}}, Congratulations, you have received a new order! ";
    }
    $message = str_replace("{{name}}", $user->display_name, $message);
    $deviceToken = get_user_meta($order_seller_id, 'shopimint_device_token', true);
    if (isset($deviceToken) && $deviceToken != false) {
        pushNotification($title, $message, $deviceToken);
    }
    $managerDeviceToken = get_user_meta($order_seller_id, 'shopimint_manager_device_token', true);
...
{
    $website = get_home_url();
    $code = get_option('shopimint_purchase_code_key');
    $response = wp_remote_post(DEACTIVE_API . "?token=" . ACTIVE_TOKEN, ["body" => ["code" => $code, "website" => $website]]);
    $statusCode = wp_remote_retrieve_response_code($response);
    $success = $statusCode == 200;
    if ($success) {
        update_option("shopimint_purchase_code", false);
        update_option("shopimint_purchase_code_key", "");
    } else {
        $body = wp_remote_retrieve_body($response);
        $body = json_decode($body, true);
        return $body["error"];
