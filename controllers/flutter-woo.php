...
class FlutterWoo extends FlutterBaseController
{
    ...
    public function register_flutter_woo_routes()
    {
		...
		register_rest_route($this->namespace, '/cart1', array(
            array(
                'methods' => "GET",
                'callback' => array($this, 'get_cart_1'),
                'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
            ),
		...
		register_rest_route($this->namespace, '/generate_cart', array(
            array(
                'methods' => "GET",
                'callback' => array($this, 'generate_cart'),
                'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
            ),
		...
		register_rest_route($this->namespace, '/getallcarttest', array(
            array(
                'methods' => "GET",
                'callback' => array($this, 'get_all_carts'),
                'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
            ),
		...
		register_rest_route($this->namespace, '/notify_abandoned', array(
            array(
                'methods' => "GET",
                'callback' => array($this, 'send_abandoned_notifications'),
                'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
            ),
	...
	public function get_cart_1($request){
		...
		$cart_items = $session['cart_totals'];
    ...
    public function get_cart($request)
    {
		...
		$cart_totals = maybe_unserialize($session['cart_totals']);
        ...
        foreach ($cart_items as $cart_item_key => $cart_item) {
            ...
            $items[] = ["product" => $product_data, "quantity" => $quantity, "variation" => $variation_data, "cart_totals" => $cart_totals];
        }
		...
		if($user) {
			if (!is_user_logged_in()) {
				...
				wp_set_auth_cookie($user_id);
 				...
 				// $url = 'https://wp.shopimint.com/checkout/';
	...
	public function get_all_carts(){
		...
		foreach($users as $user){
			...
			$user->fcm_token = get_user_meta( $user_id, 'fcm_token', true );
			// Loop through cart items and get cart items details
// 			foreach( $cart_items as $cart_item_key => $cart_item ) {
// 				$product_id   = $cart_item['product_id'];
// 				$variation_id = $cart_item['variation_id'];
...
// 			}
    ...
    public function sync_cart_from_mobile($request){
		...
		$results = $wpdb->get_results(sprintf('SELECT * FROM `%2$s` WHERE cart_ignored = 0 AND recovered_cart = 0 AND `user_id` = %d ', $user_id, $table_name));
		$new_cart = 1;
		foreach( $results as $result ) {
			if(floatval($result->cart_total) == floatval($cart_totals['total'])){
				$new_cart = 0;
			}else if(floatval($cart_totals['total'] == 0)){
				$new_cart = 0;
				$clearcart_sql = "UPDATE `$table_name` SET cart_ignored = 1, ignored_reason = 'Cart Cleared by User' WHERE id = $result->id ";
				$wpdb->get_results($clearcart_sql);
			}else{
				$new_cart = 1;
				$wpdb->get_results( "DELETE FROM `$table_name` WHERE id = $result->id " );
			}
		}
		...
		if($new_cart == 1){
			$wpdb->insert($table_name, array(
				...
				'checkout_link' => 'nolink',
				'user_email_id' => $user->data->user_email,
			));
		}
	...
	public function send_abandoned_notifications(){
		
		global $wpdb;
		$first_reminder_time = 10;
		$second_reminder_time = 120;
		$third_reminder_time = 180;
		$expired_time = 1;
		
		$table_name = $wpdb->prefix . "shopimint_abandoned_carts";
		
		$first_sql = "SELECT * FROM `$table_name` WHERE remind_count = 0 AND cart_ignored = 0 AND recovered_cart = 0 AND DATE_ADD(`cart_time`, INTERVAL $first_reminder_time MINUTE) < NOW()";
		$first_reminders = $wpdb->get_results($first_sql);
		
		$second_sql = "SELECT * FROM `$table_name` WHERE remind_count = 1 AND cart_ignored = 0 AND recovered_cart = 0 AND DATE_ADD(`cart_time`, INTERVAL $second_reminder_time MINUTE) < NOW()";
		$second_reminders = $wpdb->get_results($second_sql);
		
		$third_sql = "SELECT * FROM `$table_name` WHERE remind_count = 2 AND cart_ignored = 0 AND recovered_cart = 0 AND DATE_ADD(`cart_time`, INTERVAL $third_reminder_time MINUTE) < NOW()";
		$third_reminders = $wpdb->get_results($third_sql);
		
		$expired_sql = "UPDATE `$table_name` SET cart_ignored = 1, ignored_reason = 'Cart Expired' WHERE remind_count > 3 AND recovered_cart = 0 AND DATE_ADD(`cart_time`, INTERVAL $expired_time MINUTE) < NOW()";
		$expired_rows = $wpdb->get_results($expired_sql);
...
// 			"data" => ["title" => $title, "body" => $noti_message , "click_action" => "FLUTTER_NOTIFICATION_CLICK"], 
// 			"to" => $deviceToken
// 		];
...
// 			'third_sql' => $third_sql,
			'expired_sql' => $expired_sql,
			'first_reminders' => $first_reminders,
			'second_reminders' => $second_reminders,
			'third_reminders' => $third_reminders,
		];
		
	}
	
	public function update_abandoned_notifications_history(){
		
		$sendnotification_ids = "12";
		
		global $wpdb;
		$table_name = $wpdb->prefix . "shopimint_abandoned_carts";
		
		$sql = "UPDATE `$table_name` SET remind_count = remind_count + 1, last_remind_time = NOW() WHERE id IN ( $sendnotification_ids )";
	...
	function get_categories($request){
		...
		foreach ($all_categories as $cat) {
			...
			if($cat->category_parent == 0) {
				...
				if($sub_cats) {
					$cat->hassubcategory = true;
					foreach($sub_cats as $sub_category) {
						// echo  $sub_category->name ;
					}   
				}
				$cat->sub_category = $sub_cats;
			}       
		}
