...
class FlutterUserController extends FlutterBaseController
{
    ...
    function createSocialAccount($email, $name, $firstName, $lastName, $userName, $avatar)
    {
    ...
    public function google_login($request)
    {
        ...
        if (isset($result["email"])) {
            ...
            return $this->createSocialAccount($email, $display_name, $firstName, $lastName, $user_name, $avatar);
        } else {
    ...
    public function test_push_notification()
    {
        ...
        if($is_onesignal){
            $status = one_signal_push_notification("ShopimintApp", "Test push notification", array($user_id));
            return ['status' => $status];
        }
        if (isset($is_manager)) {
            if ($is_manager) {
                $deviceToken = get_user_meta($user_id, 'shopimint_manager_device_token', true);
                if ($deviceToken) {
                    $status = pushNotification("ShopimintApp", "Test push notification", $deviceToken);
                }
            }
        ...
        if (isset($is_delivery)) {
            if ($is_delivery) {
                $deviceToken = get_user_meta($user_id, 'shopimint_delivery_device_token', true);
                if ($deviceToken) {
                    $status = pushNotification("ShopimintApp", "Test push notification", $deviceToken);
                }
            }
        ...
        $deviceToken = get_user_meta($user_id, 'shopimint_device_token', true);
        if ($deviceToken) {
            $status = pushNotification("ShopimintApp", "Test push notification", $deviceToken);
        }
        return ["deviceToken" => $deviceToken, 'serverKey' => $serverKey, 'status' => $status];
