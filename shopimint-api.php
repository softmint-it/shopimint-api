...
// use ShopimintCheckout\Templates\MDetect;
...
class ShopimintCheckOut
{
    ...
    public function __construct()
    {
         ...
         */
        function shopimint_frontend_script()
        {
            wp_enqueue_script('my_script', plugins_url('assets/js/shopimint-woo.js', SHOPIMINT_PLUGIN_FILE), array('jquery'), '1.0.0', true);
            wp_localize_script('my_script', 'MyAjax', array('ajaxurl' => admin_url('admin-ajax.php')));
        }

        add_action('wp_enqueue_scripts', 'shopimint_frontend_script');
        // Setup Ajax action hook
        add_action('wp_ajax_shopimint_delete_json_file', array($this, 'shopimint_delete_json_file'));
        add_action('wp_ajax_shopimint_update_limit_product', array($this, 'shopimint_update_limit_product'));
        add_action('wp_ajax_shopimint_update_firebase_server_key', array($this, 'shopimint_update_firebase_server_key'));
        add_action('wp_ajax_shopimint_update_new_order_title', array($this, 'shopimint_update_new_order_title'));
        add_action('wp_ajax_shopimint_update_new_order_message', array($this, 'shopimint_update_new_order_message'));
        add_action('wp_ajax_shopimint_update_status_order_title', array($this, 'shopimint_update_status_order_title'));
        add_action('wp_ajax_shopimint_update_status_order_message', array($this, 'shopimint_update_status_order_message'));
    ...
    function shopimint_delete_json_file(){
        $id = sanitize_text_field($_REQUEST['id']);
        $nonce = sanitize_text_field($_REQUEST['nonce']);
        FlutterUtils::delete_config_file($id, $nonce);
    }

    function shopimint_update_limit_product()
    {
        $limit = sanitize_text_field($_REQUEST['limit']);
        if (is_numeric($limit)) {
            update_option("shopimint_limit_product", intval($limit));
        }
    }

    function shopimint_update_firebase_server_key()
    {
        $serverKey = sanitize_text_field($_REQUEST['serverKey']);
        update_option("shopimint_firebase_server_key", $serverKey);
    }

    function shopimint_update_new_order_title()
    {
        $title = sanitize_text_field($_REQUEST['title']);
        update_option("shopimint_new_order_title", $title);
    }

    function shopimint_update_new_order_message()
    {
        $message = sanitize_text_field($_REQUEST['message']);
        update_option("shopimint_new_order_message", $message);
    }

    function shopimint_update_status_order_title()
    {
        $title = sanitize_text_field($_REQUEST['title']);
        update_option("shopimint_status_order_title", $title);
    }

    function shopimint_update_status_order_message()
    {
        $message = sanitize_text_field($_REQUEST['message']);
        update_option("shopimint_status_order_message", $message);
    }
    ...
    public function handle_received_order_page()
    {
        ...
        if (is_order_received_page()) {
            $detect = new MDetect;
            if ($detect->isMobile()) {
                wp_register_style('shopimint-order-custom-style', plugins_url('assets/css/shopimint-order-style.css', SHOPIMINT_PLUGIN_FILE));
                wp_enqueue_style('shopimint-order-custom-style');
            }
        }
